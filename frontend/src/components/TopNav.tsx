    ...
    <header className="fixed top-0 left-0 right-0 z-50 glass-header bg-surface/80 h-14 md:pl-64 border-b border-outline-variant/10">
      <div className="h-full flex justify-between items-center px-6">
        <div className="flex items-center gap-4 min-w-0">
          <h1 className="text-sm font-semibold tracking-tight text-primary truncate">
            {title}
          </h1>
          {subtitle && (
            <span className="text-micro font-bold uppercase tracking-[0.05rem] text-on-surface-variant truncate hidden sm:block">
              {subtitle}
            </span>
          )}
        </div>
